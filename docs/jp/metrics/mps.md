...
**シナリオ：全バンド相関 ≥0.95、THD+N と TFS も優秀**
- 結論：デバイスは定常および動的内容の両方を保存；高忠実度の可能性
...
| `pink-noise` (自然) | > 0.88 | 全般的な包絡忠実度；変調信号より感度低い |
...
2. **劣化版をシミュレート**（例：16 kHz ローパスフィルタ）：
   ```bash
   # sox または独自 DSP で dut_modulated.wav を作成
